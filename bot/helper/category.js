...
const {get_all_users} = require("./users");


const get_all_categories = async (msg) => {
    const chatId = msg.from.id
    let user = await User.findOne({chatId}).lean()
    let categories = await Category.find().skip(0).limit(10).lean()
    ...
    bot.sendMessage(chatId, 'Kategoriyalar ro`yxati', {
        reply_markup: {
            remove_keyboard: true, inline_keyboard: [...list, [{
                text: 'Ortga', callback_data: 'back_category'
            }, {
                text: 1, callback_data: '0'
            }, {
                text: 'Keyingi', callback_data: 'next_category'
            }], user.admin ? [{
